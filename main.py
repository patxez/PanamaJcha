...
async def generate_transcript(ch, u, cb, cat):
    ...
    for m in msgs:
        ...
        for a in m.attachments:
            ...
            else: h += f"<div class='txt'><a href='{a.url}' style='color:#00aff4;'>ไฟล์: {a.filename}</a></div>"
        h += "</div></div>"
    return h + "</body></html>"
...
async def update_member_status(did, rid, rn, gid=None):
    ...
    try:
        m = await g.fetch_member(int(did)); in_g, rv, rname = check_group_membership(rid); is_d = int(rid) in DEVELOPER_IDS
...
class CustomizeAllModal(discord.ui.Modal, title="ปรับแต่งระบบทั้งหมด"):
    ...
    pxs = discord.ui.TextInput(label="คำนำหน้า (OF-3=MAJ; OF-4=LTC)", style=discord.TextStyle.paragraph, required=False)
    async def on_submit(self, it: discord.Interaction):
        s = load_settings()
        try:
...
async def close_t(it: discord.Interaction):
    ...
    await it.response.send_message("🔒 บันทึกประวัติและปิด Ticket..."); s = load_settings(); u = it.guild.get_member(int(row["user_id"]))
...
async def set_ts(it: discord.Interaction, ช่อง: discord.TextChannel):
    ...
    await it.response.send_message(f"✅ ตั้งค่าช่องประวัติเป็น {ช่อง.mention}", ephemeral=True)
...
@bot.tree.command(name="ใส่คำนำหน้า")
@app_commands.default_permissions(administrator=True)
async def set_px(it: discord.Interaction, ยศ: str, คำนำหน้า: str):
    s = load_settings(); s["rank_prefixes"][ยศ.strip().lower()] = f"{ยศ.strip()}, {คำนำหน้า.strip()}"; save_settings(s)
    await it.response.send_message(f"✅ เพิ่มคำนำหน้า {ยศ} แล้ว", ephemeral=True)

@bot.tree.command(name="ปรับแต่งทั้งหมด")
@app_commands.default_permissions(administrator=True)
async def cust(it: discord.Interaction): await it.response.send_modal(CustomizeAllModal())

@bot.tree.command(name="ดูการตั้งค่า")
@app_commands.default_permissions(administrator=True)
async def show_s(it: discord.Interaction):
    s = load_settings(); r = s.get("role_ids", {})
    em = discord.Embed(title="การตั้งค่า", color=0x3498DB)
